...
// Payout breakdown for the Buy-in article - a dense single-line string was
// hard to scan, so this renders each payout as its own row (label left,
// dollar amount right, matching the budget-list styling) with a running
// total in the heading.
function payoutsHTML(payouts) {
    const total = payouts.reduce((sum, p) => sum + p.amount, 0);
    return `
        <div class="payouts-block">
              <div class="payouts-heading">Payouts <span class="payouts-total">&middot; $${total.toLocaleString()} total</span></div>
                    ${payouts
                              .map(
                                          (p) => `
