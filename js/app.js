...
// Keeper helpers. KEEPERS_2026[slug] is a list of { player, salary }, where
// salary is the final keeper fee already applied - not the original draft
// price. "Live" budget is what Draft & budget in the Constitution defines
// as the number that actually matters at the table: trade-adjusted cap
// (BUDGETS_2026) minus total keeper cost.
function keepersFor(slug) {
        return KEEPERS_2026[slug] || [];
}

function keeperCost(slug) {
        return keepersFor(slug).reduce((sum, k) => sum + k.salary, 0);
}

function liveBudget(slug) {
        return BUDGETS_2026[slug] - keeperCost(slug);
}
                                                                ...
                                                                <div class="label">Keeper deadline</div>
                                                                        <div class="value">${Math.max(keeperDays, 0)} days</div>
                                                                                <div class="sub">Sun, Sep 6</div>
                                                                                              ...
                                                                                              <h2 class="section-title">2026 live draft budgets</h2>
                                                                                                  <p class="helper-note" style="margin-bottom:8px;">After keepers - what's actually left to spend at the table on Sep 7.</p>
                                                                                                      <div class="card">
                                                                                                            ${sortedBudgets
                                                                                                                      .map((slug) => {
                                                                                                                                      ...
                                                                                                                                      return `<div class="budget-row">
                                                                                                                                                  <span class="owner-link" data-nav="owners/${slug}">${o.name}</span>
                                                                                                                                                              <span class="amount">$${liveBudget(slug)}</span>
                                                                                                                                                                        </div>`;
                                                                                                                            })
                                                                                                                      .join("")}
                                                                                                                          </div>
...
function renderOwnerProfile(slug) {
        ...
        const keepers = keepersFor(slug);
        const spent = keeperCost(slug);
        const live = budget - spent;
        app.innerHTML = `
                                                                                                                ...
                                                                                                                <div class="label">2026 draft budget</div>
                                                                                                                        <div class="value" style="font-size:18px;color:var(--green-dark);">$${budget}</div>
                                                                                                                                <div class="sub">before keeper costs</div>
                                                                                                                                            ...
                                                                                                                                            <div class="stat-tile">
                                                                                                                                                    <div class="label">Live draft budget</div>
                                                                                                                                                            <div class="value" style="font-size:18px;color:var(--green-dark);">$${live}</div>
                                                                                                                                                                    <div class="sub">${keepers.length ? `after $${spent} on ${keepers.length} keeper${keepers.length === 1 ? "" : "s"}` : "no keepers this year"}</div>
                                                                                                                                                                          </div>
                                                                                                                                                                                <div class="stat-tile">
                                                                                                                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                                                                                                                      <div class="card">
                                                                                                                                                                                                                                                                                                                            ${
                                                                                                                                                                                                                                                                                                                                          keepers.length
                                                                                                                                                                                                                                                                                                                                            ? keepers
                                                                                                                                                                                                                                                                                                                                                .map(
                                                                                                                                                                                                                                                                                                                                                                      (k) => `<div class="budget-row"><span>${k.player}</span><span class="amount">$${k.salary}</span></div>`
                                                                                                                                                                                                                                                                                                                                                                    )
                                                                                                                                                                                                                                                                                                                                                .join("")
                                                                                                                                                                                                                                                                                                                                            : `<p class="helper-note" style="margin:0;">No keepers designated for 2026.</p>`
                                                                                                                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                                                                                                                            </div>
...
// Payout breakdown - a dense single-line string was hard to scan, so this
// gets its own section (heading + card, same pattern as "2026 draft
// budgets") rather than being squeezed inside the Buy-in article's card.
// Each payout is its own row (label left, dollar amount right) with a
// running total in the heading.
function payoutsHTML(payouts) {
        const total = payouts.reduce((sum, p) => sum + p.amount, 0);
        return `
            <h2 class="section-title">Payouts <span class="payouts-total">&middot; $${total.toLocaleString()} total</span></h2>
                <div class="card">
                      ${payouts
                                .map(
                                                (p) => `
                                                        <div class="budget-row">
                                                                  <span style="color:var(--navy-mid);">${p.label}${
                                                                                    p.note ? ` <span style="font-size:11px;">(${p.note})</span>` : ""
                                                                  }</span>
                              ...
                              <div class="card">
                                      ${settingsGroupHTML(a.rows)}
                                              ${notesHTML(a.notes)}
                                                          ...
                                                          ${a.payouts ? payoutsHTML(a.payouts) : ""}
                                                              `
                ).join("");
          ...
          <p class="helper-note">How the league runs today. Tap "Why &amp; history" under any section for background; tap a season below for the full vote record.</p>
              ...
              ${articlesHTML}
                  ...
                  <h2 class="section-title">On the docket for 2026</h2>
                      ${
                                  DOCKET_2026.length
                                    ? `<p class="helper-note" style="margin-bottom:8px;">Not yet ratified — still in play this cycle.</p>${docketHTML}`
                                    : `<div class="card"><p class="helper-note" style="margin:0;">Nothing open right now — the full 2025 cycle (8 proposals) was voted on and resolved ahead of the 2026 draft. See the archive below for results. Nothing new has been nominated for the next cycle yet.</p></div>`
                      }
